...
def _build_synthesis_prompt(query: str, sources: list[dict[str, Any]], failed_sources: list[dict[str, Any]]) -> str:
    ...
    for idx, src in enumerate(sources, 1):
        ...
        # Protect against prompt injection by tagging untrusted content
    ...
    return (
        ...
        "4. Do NOT hallucinate facts, claims, or URLs that are not in the sources.\n"
        "5. If the sources do not contain enough information to answer the question, state that clearly.\n\n"
...
def create_research_skill(
    ...
    def handler(input_data: Any, context: dict[str, Any] | None = None) -> Any:
        ...
        if exec_tool is not None:
            ...
            sources_data = [{"title": s.title, "url": s.url, "snippet": s.snippet, "content": s.content, "domain": s.source_domain} for s in report.sources]
        ...
        if synthesize and model is not None and sources:
            prompt = _build_synthesis_prompt(query, sources, failed_sources)
            try:
