...
from research.url_utils import is_safe_url, normalize_url, resolve_and_validate_ip

logger = logging.getLogger("aura.research.providers.browser")


def _is_safe_host(hostname: str) -> tuple[bool, str | None]:
    """Validate that host does not resolve to restricted IP ranges."""
    if not hostname:
        return False, "Missing hostname."
    clean_host = hostname.strip().lower()
    if clean_host in ("localhost", "127.0.0.1", "0.0.0.0", "169.254.169.254", "::1", "[::1]") or clean_host.endswith(".local") or clean_host.endswith(".internal"):
        return False, f"Access to restricted host '{clean_host}' is blocked."
    return resolve_and_validate_ip(clean_host)
    ...
    def fetch_page(
        ...
        try:
            with urllib.request.urlopen(req, timeout=effective_timeout) as resp:
                ...
                if final_url != norm_url and not self.allow_local:
            ...
            reason_str = str(ue.reason)
            if "timed out" in reason_str.lower() or isinstance(ue.reason, socket.timeout):
                return WebDocument(
                    url=norm_url,
                    title="",
                    content="",
                    status_code=504,
                    error=f"Render timed out after {effective_timeout}s.",
                    metadata={"provider": self.name, "timeout": True},
                )
            return WebDocument(
                url=norm_url,
                title="",
            ...
            return WebDocument(
                ...
                content="",
                status_code=500,
                error=f"Dynamic render failed: {str(e)}",
                metadata={"provider": self.name},
            )
