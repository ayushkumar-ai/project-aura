...
from research.url_utils import is_safe_url, normalize_url
...
class BoundedWebCrawler:
    ...
    def crawl(
        self,
        query: str,
        seed_items: list[SearchItem] | tuple[SearchItem, ...] | None = None,
        ...
        """Execute bounded multi-hop traversal starting from seed search items or resumed checkpoint."""
        ...
        # Step 0: Restore from Checkpoint if provided
        if checkpoint is not None:
            if not isinstance(checkpoint, ResearchCheckpoint):
                raise TypeError("checkpoint must be a ResearchCheckpoint instance.")
            visited_urls.update(checkpoint.visited_urls)
            successful_sources.extend(checkpoint.successful_sources)
            failed_sources.extend(checkpoint.failed_sources)
            all_discovered_links.extend(checkpoint.discovered_links)
            total_chars_accumulated = checkpoint.accumulated_chars
            total_fetches_count = checkpoint.total_fetches
            queued_links.extend(checkpoint.pending_links)

        # Step 1: Initialize queue with Hop 0 seeds if provided
        if seed_items:
        ...
        while (
            ...
            is_safe, err_reason = is_safe_url(target_url)
            if not is_safe:
        ...
        stats = {
            ...
            "pending_links_count": len(queued_links),
        ...
        ckpt = ResearchCheckpoint(
            ...
            discovered_links=tuple(all_discovered_links),
            pending_links=tuple(queued_links),
            accumulated_chars=total_chars_accumulated,
            total_fetches=total_fetches_count,
            traversal_stats=dict(stats),
